...
from practical_deps.sweagent.agent.commands import Command, ParseCommand
from practical_deps.sweagent.agent.history_processors import HistoryProcessor
from practical_deps.sweagent.agent.models import (
    APIStats,
...
from practical_deps.sweagent.agent.parsing import FormatError, ParseFunction
from practical_deps.sweagent.agent.summarizer import SummarizerConfig
from practical_deps.sweagent.environment.swe_env import SWEEnv
from practical_deps.sweagent.types import AgentInfo, History, HistoryItem, Trajectory, TrajectoryStep
from practical_deps.sweagent.utils.config import convert_paths_to_abspath
from practical_deps.sweagent.utils.log import get_logger
...
class AgentConfig(FrozenSerializable):
    ...
    system_template_flag_format: str | None = None
...
class Agent:
    ...
    def traj_path(self) -> Path | None:
        ...
        if self.traj_dir and self._env is not None:
            ...
            if self._env.record.get("step_id") is not None:
                return self.traj_dir / (self._env.record["instance_id"] + f"_{self._env.record.get('step_id')}" + ".traj")
    ...
    def replay_actions(self, env: SWEEnv, actions: list[str]) -> None:
        """Replay prior actions to restore environment state before a subtask."""
        ...
        for action in actions:
            ...
            for sub_action in self.split_actions(run_action):
                if sub_action["cmd_name"] == self.config.submit_command:
                    continue
                if sub_action["agent"] != self.name:
                    self.logger.warning("Skipping subroutine replay for %s", sub_action["agent"])
                    continue
                observation, _, _, info = env.step(sub_action["action"])
    ...
    def setup_attempt(self, *, init_model_stats: APIStats | None = None) -> None:
        ...
        if "flag_format" in self.instance_args:
            system_msg = self.config.system_template_flag_format.format(**self.system_args, **self.instance_args)
        else:
    ...
    def forward_model(self, observation: str | None, state: str) -> str:
        ...
        self.logger.debug(f"state_vars: {state_vars}")
        templates: list[str] = []
    ...
    def _run_step(self, observation: str | None) -> tuple[str | None, bool]:
        ...
        self.logger.debug(f"state_command: {self.state_command}")
    ...
    def run(
        ...
        if setup_args.get("subtask_history") is None or not setup_args.get("subtask_history"):
        ...
        elif env.container_obj.id != self.last_container_id:
            self.last_container_id = env.container_obj.id
