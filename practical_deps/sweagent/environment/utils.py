...
from practical_deps.sweagent.agent.interactive_commands import InteractiveSession
        ...
        if ready_to_read(fd):
            ...
            if data:
                ...
                if terminal_pattern in buffer.decode("utf-8", errors="backslashreplace").replace("\r\n", "\n"):
                    ...
                    break
        time.sleep(0.01)  # Prevents CPU hogging
    logger.debug("Finished timeout read")
    decoded = buffer.decode("utf-8", errors="backslashreplace").replace("\r\n", "\n")
...
class InstanceBuilder:
    ...
    def set_problem_statement_from_json(self, file_path: Path):
    ...
    def set_problem_statement_from_file(self, file_path: str):
        if Path(file_path).name == "challenge.json":
            ...
            self.set_problem_statement_from_json(file_path)
        else:
    ...
    *,
    ...
    if (
        ...
        or (
            ...
            and (Path(file_path).suffix in [".md", ".txt", ".json"] or Path(file_path).name == "challenge.json")
        )
